...
# ✅ Now we store only 3 most recent errors, each as dict with timestamp
...
def log_error(message: str):
    """Helper to add error with timestamp."""
    with errors_lock:
        errors.append({
...
def topic_exists_check(topic_name, bootstrap_server):
    try:
        ...
        log_error(f"Admin client error: {str(e)}")
        return False
...
def health_updater():
    ...
    while True:
        ...
        # ✅ Filter recent errors (within last 5 mins)
        now = datetime.now()
        with errors_lock:
            valid_errors = [
                e for e in errors
                if now - datetime.strptime(e["time"], "%Y-%m-%d %H:%M:%S") <= timedelta(minutes=5)
            ]
        detail['error'] = valid_errors

        # ✅ Determine status
        detail['status'] = "OK" if topic_exist and not valid_errors else "ERRO"

        # ✅ Handle missing topic case
        if not topic_exist:
            log_error(f"Topic '{current_topic}' does not exist on the broker.")

        with lock:
            detail['latest_records'] = latest_records.copy()
        detail['last_refreshed'] = now.strftime("%Y-%m-%d %H:%M:%S")
    ...
    try:
        ...
        while True:
            try:
                ...
                for tp, messages in msg_pack.items():
                    for message in messages:
                        try:
                            ...
                            log_error(f"Thread-{thread_num}: error processing message: {str(e)}")
                ...
                log_error(f"Thread-{thread_num}: error in poll {str(e)}")
        consumer.close()
    except Exception as e:
        log_error(f"Thread-{thread_num}: startup error {str(e)}")
    finally:
...
def health():
    now = datetime.now()
    with errors_lock:
        # Get errors from your deque that are within the last 5 minutes
        recent_errors = [e for e in errors if now - datetime.strptime(e['time'], "%Y-%m-%d %H:%M:%S") < timedelta(minutes=5)]
    with status_lock:
        response = health_status.copy()
        if not recent_errors:
            return jsonify({"status": "success"})
        else:
            return jsonify({"status": "degraded", "errors": recent_errors})
...
def main():
    updater_thread = threading.Thread(target=health_updater, daemon=True)
    ...
    if topic_exists_check(topic_name, bootstrap_server):
        ...
        for i in range(NUM_CONSUMERS):
            try:
                ...
                log_error(f"Main thread error: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                log_error(f"Error writing CSV: {str(e)}")
